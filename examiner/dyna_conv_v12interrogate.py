# 1) follow up question can be after any of the 3 types. Add example like "Are you sure XXXX", "I actually see a XXX around/.. {co-occur_object, you really can't see..?}"
# 2) In each of the 3 question types not followup, stress that please don't provide a question that repeats or has the same meaning as any of the prior questions
# 3) "gt": "xxxx" is not in prompt example or required to be returned for these 2 types:, Just default unanswerable to I can't answer the question because the object doesn't exist. Default adversial answer to No
...
Please generate two contexts based on the image information. Please make sure the contexts involve limited specific descriptions of object features in the image and stay as high-level glimpses without leaking too many observatory details. Make sure the contexts are diverse and not redundant.
...
2. **Follow-up questions** – Follow-up questions could be triggered when the model correctly answered the last question (any type). The purpose of the follow-up question is to test the confidence of the model's answer or to ask for further challenging details when available.
...
The purpose of the follow-up question is to interrogate the confidence of the model's answer despite misleading subjective evidence or to ask for further challenging details when available.
...
4) **CRITICAL: Do NOT repeat or ask questions with the same meaning as any prior questions in the conversation history.**


Please output the hallucinated scenario/object as scene graph elements"names"/"attributes"/"relations", a list of up to 5 visible features it co-occurs with, and the corresponding question in the form of a json dict.

Examples:
If there is a blue banana on the table while banana should usually be yellow.
```json
{{"names": "banana", "attributes": "yellow", "co-occur_with": "banana, yellow", "question": "Is there a yellow banana present on the table?"}}
```
If there is cake on the table, which is usually accompanied by a knife but currently isn't.
```json
{{"names": "knife", "co-occur_with": "cake, table, plate", "question": "Is there a knife present on the table?"}}
```

Allowed keys in the JSON output are "names", "attributes", "relations", "co-occur_with", and "question".
"""
...
class EvalSample:
    ...
    def select_context_nodes(self, context):
        # Extract relevant_objects from context (already selected during context generation)
        selected_node_ids = context.get("relevant_objects", [])
        selected_object_nodes = [self.scene_graph_data.image_info.sg.get_object_by_id(int(oid)) for oid in selected_node_ids]

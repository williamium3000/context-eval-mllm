# 1) emphasize follow up question is after CORRECT answer
# 2) context node selection integrated with context gen, and add 3 timese retry for parsing error 
...
2. **Follow-up questions** – Follow-up questions could be triggered only when the model **CORRECTLY** answered the last question (any type). The purpose of the follow-up question is to test the confidence of the model's answer or to ask for further challenging details when available.
...
class EvalSample:
    ...
    def run(self):
        ...
        for context in valid_contexts:
            selected_nodes = [self.scene_graph_data.image_info.sg.get_object_by_id(int(oid)) for oid in context.get("relevant_objects", [])]

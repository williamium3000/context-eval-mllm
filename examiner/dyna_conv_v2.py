...
REGULAR_CONV_PROMPT = \
"""Based on the given image and context, please ask a regular question about the object: {}.
Please ask questions that are strongly reated to the given context and image. Avoid irrelevant or out-of-character questions.
For example, in an office setting, the goal of the conversation is to send an email to his boss. you can ask questions like "is the monitor turned on?", "where is the power button of the computer?", "I want to type in the email, what should I look for?" etc.
...
class EvalSample:
    ...
    def ask_regular(self, conversations, selected_nodes, context):
        ...
        conversations.append({"role": "user", "content": REGULAR_CONV_PROMPT.format(sampled_node)})

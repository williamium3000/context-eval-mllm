...
class GoogleSerperAPIWrapper():
    ...
    async def run(self, queries):
        ...
        for i in range(len(results)):
            # Check if the result is an exception (from asyncio.gather with return_exceptions=True)
            if isinstance(results[i], Exception):
                # Handle the exception by creating an empty result
                print(f"Search failed with error: {results[i]}")

...
import torch
import json
from decoder_zoo.VASparse.detector_masker import Detector
...
class vasparse_assistant:
    ...
    def visual_aware_region_embedding(self, entity):
        ...
        if detect_info["pos"] in valid_list:
            ...
            cropped_images = []
            try:
                masked_image = sample["entity_info"][entity].get('masked_image', None)
            except:
                masked_image = None
            if masked_image is not None:
                cropped_images.append(original_image)
